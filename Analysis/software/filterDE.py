...
DATA_FOLDER = os.path.normpath(os.path.join(BASE_DIR, "..", "dataset", "DE"))
OUTPUT_FOLDER = os.path.normpath(os.path.join(BASE_DIR, "pos_listsDE"))
...
def extract_lemmas_and_pos_from_file(file_path):
    ...
    try:
        ...
        for w in w_elements:
            ...
            # Extract lemma
            lemma_elem = w.xpath("txm:ana[@type='#delemma']", namespaces=ns)
            lemma = lemma_elem[0].text.strip() if lemma_elem and lemma_elem[0].text else token

            # Extract POS
            pos_elem = w.xpath("txm:ana[@type='#depos']", namespaces=ns)
...
meaningful_pos = {
    "NN", "NE", "TRUNC",    # nominal forms
    "VVFIN", "VVINF", "VVPP", "VVIMP", "VVIZU",  # verbs
